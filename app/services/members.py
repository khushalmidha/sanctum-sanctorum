...
def get_member_stats(db: Session, member_id: int, now: datetime) -> MemberStats:
    ...
    # Order stats: only paid orders.
    paid_orders = list(
        db.scalars(
            select(Order)
            .where(Order.member_id == member.id, Order.status == OrderStatus.PAID.value)
        )
    )
    ...
    # Loan stats.
    loans = list(db.scalars(select(Loan).where(Loan.member_id == member.id)))
    active_loans = sum(1 for loan in loans if loan.returned_at is None)
    overdue_loans = sum(
        1 for loan in loans if loan.returned_at is None and now > loan.due_at
    )
